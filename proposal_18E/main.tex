...
We will design and implement the provenance-first agentic KG architecture with three integrated layers: knowledge representation; automation and extraction; and agentic orchestration (see Fig~\ref{fig:pa-akg}). PA-AKG will be designed as a deployable knowledge-layer component, positioned between raw HPC artifacts and AI reasoning. The structured provenance KG will promote accelerated and trustworthy DOE AI-assisted R\&D workflows, scalable across the Genesis Mission portfolio. The initial implementation will focus on software engineering within HPC and scientific contexts~\cite{basili2008understanding}. Software engineering is critical for technological innovation~\cite{vos2026reclaiming}, scientific productivity~\cite{osti_1606662} and enabling HPC applications~\cite{schmidberger2012need}, directly impacting the quality and reliability of scientific software~\cite{mundt2025pyomo}. Research scientists view software as instrumental to their work~\cite{hettrick}, yet find development processes inhibit scientific discovery~\cite{basili2008understanding,carver2013self}. Further, Pope suggests DOE workflows and teams should adhere to software engineering best practices~\cite{pope2015future}. 

We will target \textbf{Pyomo}~\cite{bynum2021pyomo}, an open-source optimization framework, as the primary use case for our initial implementation---investigating its codebase,\footnote{\url{https://github.com/pyomo/pyomo}} research literature, and workflows at Sandia National Laboratories~\cite{milewicz2020research}. We will benchmark PA-AKG against RAG, GraphRAG~\cite{Edge2024GraphRAG}, and static baselines on provenance completeness, reproducibility, efficiency, and trustworthiness of AI-generated software decisions. We will also conduct pilot and controlled user studies with HPC software developers to gain insights on our approach and its applicability for HPC research and development workflows. Our project will be guided by an \textbf{advisory board} of HPC experts and researchers, who will hold regular meetings with the research team throughout the project period. Key risks, including the extraction reliability at HPC code scale and KG schema portability across domains, will be mitigated by the staged evaluation design. \textit{This project does not involve the construction, alteration, maintenance, and/or repair of public infrastructure in the United States.}
...
\textit{Evaluation:} We will run an initial \textbf{small-seed calibration study} of the PA-AKG pipeline against a small seed corpus of approximately 5--10 relevant research papers, beginning with relevant work (\ie~\cite{basili2008understanding,mundt2025pyomo,schmidberger2012need}) as the canonical seed and snowballing backward through its references---curated by the research team. The study will proceed in two modes: \textit{discovery}, in which the team hand-annotates claim--evidence pairs to establish baseline thresholds for provenance completeness and span-level alignment; and \textit{calibration}, in which those thresholds are refined to the empirical cutoff that distinguishes trustworthy from untrustworthy recommendations. Annotation will be performed by consensus, cross-checked across members of the research team. These findings will inform the Decision Gate Metrics at the Month-6 gate. Concurrently, we will conduct a \textbf{pilot study} with researchers to gain formative insights on the small-seed PA-AKG corpus. 
...
Sandia National Laboratories’ Center for Computing Research (CCR) creates technology and solutions for many of our nation’s most demanding national security challenges. The Center’s portfolio spans the spectrum from fundamental research to state of the art applications. CCR work includes: computer system architecture (both hardware and software); enabling technology for modeling physical and engineering systems; and research in discrete mathematics, data analytics, cognitive modeling, and decision support materials. CCR researchers are primarily housed in the Computer Science Research Institute (CSRI), located in the Sandia Science and Technology Park just outside Kirtland Air Force Base. The 34,500-square foot CSRI building combined with other offices distributed throughout the base contain space for nearly 300 researchers, staff, and visitors and include several conference rooms and collaborative workspaces. These facilities contain local computing resources and have high-bandwidth network connections to Sandia’s high-performance computing platforms, several of which are provided at no cost to individual projects.
...
\item As an invited speaker for the Consortium for the Advancement of Scientific Software (CASS) User-Developer Experience, I presented my research on designing for development tools for research software engineers to an audience of software practitioners at the practitioner-focused working group.
...
\begin{enumerate}
    \item Dr. Xuan Wang has been serving on program committees for prestigious conferences in natural language processing (ACL, EMNLP, NeurIPS), data mining (KDD, TheWebConf), and bioinformatics (ISMB). 
    
    \item Dr. Xuan Wang has been serving as journal reviewer for highly reputed journals in data mining (ACM TIST) and biomedical informatics (Nature Methods, Nature Communications, Cell).
    
    \item Dr. Xuan Wang has been serving as the program chair for several workshops and symposiums, including the South NLP Symposium (2024) and the “Undergraduate and High School Symposium” at BigData’24 and ICDM’25.
    
    \item Dr. Xuan Wang has served as a mentor in the “Women in Machine Learning Workshop” (co-located with KDD’22) and participated in the "1st Global Women in NLP" workshop, promoting awareness and appreciation of women's achievements in machine learning and NLP communities.
    
    \item Dr. Xuan Wang has delivered tutorials on scientific text mining at prominent conferences such as KDD’22, TheWebConf’22, and BigData’19, and recently delivered a new tutorial on AI for science with large language models at AAAI’25 and EMNLP’24.
...
\subsubsection*{Letters of Commitment and Letters of Collaboration}\label{app:letters}}

\includepdf[scale=1.0,offset=1in -1in,pages=-,width=8in]{letters/sandia_commit.pdf}

\includepdf[scale=1.0,offset=1in -1in,pages=-,width=8in]{letters/vt.pdf}

\includepdf[scale=1.0,offset=1in -1in,pages=-,width=8in]{letters/dsn.pdf}

\includepdf[scale=1.0,offset=1in -1in,pages=-,width=8in]{letters/miranda.pdf}

\includepdf[scale=1.0,offset=1in -1in,pages=-,width=8in]{letters/reed.pdf}

\includepdf[scale=1.0,offset=1in -1in,pages=-,width=8in]{letters/cohoon_Collaboration.pdf}
